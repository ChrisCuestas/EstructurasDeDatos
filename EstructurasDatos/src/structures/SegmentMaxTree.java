...
public class SegmentMaxTree<T> {
    ...
    public SegmentMaxTree(double arr[], int size){
    ...
    private double constructSTUtil(double arr[], int start, int end, int currentIndex)
    {
        ...
        // right subtrees and store the maximum of values in this node
        int mid = getMid(start, end);//To split the range by the middle, obtain the middle index
        tree[currentIndex] = maxOf(constructSTUtil(arr, start, mid, currentIndex * 2 + 1),
    ...
    public double getMax(int size, int qStart, int qEnd){
        // Check for erroneous input values
        if (qStart < 0 || qEnd > size - 1 || qStart > qEnd) {
            System.out.println("Invalid Input");
            return -1;
        }
        return getMaxUtil(0, size - 1, qStart, qEnd, 0);
    }
    ...
    private double getMaxUtil(int start, int end, int qStart, int qEnd, int currentIndex){
        ...
        return maxOf(getMaxUtil(start, mid, qStart, qEnd, 2 * currentIndex + 1),
                getMaxUtil(mid + 1, end, qStart, qEnd, 2 * currentIndex + 2));
    }
    ...
    private void updateValueUtil(int start, int end, int i, double newVal, int currentIndex)
    {
        ...
        if (end != start) {
            ...
            tree[currentIndex]=maxOf(tree[2 * currentIndex + 1],tree[2 * currentIndex + 2]);
        } else {
